...
const iosWebViewController = read(
  "../../ios-native/Sources/QimingApp/QimingWebViewController.swift"
);
const nativeWorkflow = read("../../.github/workflows/linter.yml");
...
test("iOS production shell starts at login and preserves native safe areas", () => {
  assert.match(iosWebViewController, /QIMING_ENTRY[^\n]*\/login/);
  assert.match(iosWebViewController, /qimingNative=1&nativeStatusTop=/);
  assert.match(iosWebViewController, /--pure-safe-area-top/);
  assert.doesNotMatch(
    iosWebViewController,
    /demoRole|qiming-demo-role|testScript/
  );
  assert.match(
    globalStyles,
    /qiming-native-ios[\s\S]*--qiming-native-effective-safe-top/
  );
