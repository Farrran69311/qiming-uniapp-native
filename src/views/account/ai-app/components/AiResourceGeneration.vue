...
const reportResourceUsage = async (
  ...
  if (isDemoTask(resource.task_id)) return false;
  try {
...
const retryTaskList = async () => {
  ...
  taskLoadError.value = "";
  try {
    const { data } = await listAssistantResourceTasks({
      course_id: props.courseId,
      target_student_id: props.targetStudentId
    });
...
const loadResources = async () => {
  ...
  try {
    ...
    if (taskResult.status === "fulfilled") {
      mergeServerTasks(taskResult.value?.data?.list || []);
    } else {
      taskLoadError.value = assistantApiErrorMessage(
        taskResult.reason,
        "任务列表加载失败，请重试"
...
const loadTaskActivity = async (taskId: string) => {
  ...
  try {
    ...
    if (logsResult.status === "fulfilled") {
      taskLogs.value = logsResult.value?.data?.list || [];
    }
    if (traceResult.status === "fulfilled") {
      taskTrace.value = traceResult.value?.data?.trace || [];
    }
...
const handleCreateTask = async () => {
  ...
  try {
    ...
    if (data.accepted !== true) {
      ElMessage.error(data.message || "服务端未接受本次资源生成任务");
      return;
    }
...
const handleDeleteResource = async () => {
  const resource = selectedResource.value;
  if (!resource || isDemoResource(resource)) return;
  if (
  ...
  try {
    const { data } = await deleteAssistantResource(resource.resource_id);
        ...
        <div class="resource-dialog__footer">
          <div class="resource-dialog__action-group">
            ...
            <el-button
              ...
              :disabled="
                isDemoResource(selectedResource) || governanceSubmitting
              "
