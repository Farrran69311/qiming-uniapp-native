...
test("starts at or below the server initial lead", () => {
  const processor = createProcessor();

  assert.equal(processor.initialBufferMs, 240);
  assert.equal(processor.rebufferMs, 480);

  appendSamples(processor, 5759);
...
test("keeps the recovery target below the server steady-state lead", () => {
  ...
  appendSamples(
    processor,
    Math.round((24000 * processor.rebufferMs) / 1000)
  );
  renderQuantum(processor);
  assert.equal(
