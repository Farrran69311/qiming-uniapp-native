...
if (!resolvedTarget.startsWith(allowedPrefix)) {
  throw new Error(`Refusing to sync outside native-app/src/hybrid: ${resolvedTarget}`);
}
...
for (const cssPath of walkFiles(targetDir, filePath => filePath.endsWith(".css"))) {
...
`;
writeFileSync(join(targetDir, "uni.webview.1.5.8.js"), uniWebviewScript, "utf8");
...
writeFileSync(join(targetDir, "qiming-native-bridge.js"), bridgeScript.trimStart(), "utf8");
writeFileSync(join(targetDir, "qiming-native-compat.js"), compatibilityScript.trimStart(), "utf8");
...
function injectHeadScript(input, scriptName) {
  ...
  const firstModuleScript = output.match(/\s*<script[^>]+type=["']module["'][^>]*><\/script>/);
